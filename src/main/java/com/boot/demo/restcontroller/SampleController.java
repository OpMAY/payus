...
import com.boot.demo.util.security.AES256Util;
import com.boot.demo.util.Decoder;
import com.boot.demo.util.security.Security;
...
public class SampleController {
    ...
    public ResponseEntity getTest(@PathVariable int no) throws JSONException {
        ...
        return new ResponseEntity(DefaultRes.res(StatusCode.TEST, message.getHashMap()), HttpStatus.OK);
    }
    ...
    public ResponseEntity Auth(@RequestParam("secret_key") String secret_key) {
        try {
            if (secret_key.equals(access_key)) {
                ...
                return new ResponseEntity(DefaultRes.res(StatusCode.TEST, message.getHashMap("Auth()")), HttpStatus.OK);
            } else {
    ...
    public ResponseEntity ValidateToken(@RequestHeader HttpHeaders headers) {
        ...
        if (new Security(access_key).validateToken(token)) {
            return new ResponseEntity(DefaultRes.res(StatusCode.TEST), HttpStatus.OK);
        } else {
            throw new AuthorizationTokenException(BusinessExceptionType.AUTHORIZATION_TOKEN_EXCEPTION);
        }
    }
